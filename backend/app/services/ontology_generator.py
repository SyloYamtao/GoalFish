...
def _ontology_ui_locale_instruction(locale: str) -> str:
    ...
    return (
        ...
        f"Examples: {examples}. "
        "Descriptions and analysis_summary are generated content and should follow the current UI-selected content language instruction."
    )
    ...
    "entity_types": [
        {
            ...
            "description": "简短描述（按当前界面语言，不超过100字符）",
            "attributes": [
                {
...
class OntologyGenerator:
    ...
    def generate(
        ...
        system_prompt = f"{base_prompt}\n\n{lang_instruction}\n\n{ui_locale_instruction}\nIMPORTANT: Entity type names in `name` MUST be in English PascalCase (e.g., 'PersonEntity', 'MediaOrganization'). Relationship type names in `name` MUST be in English UPPER_SNAKE_CASE (e.g., 'WORKS_FOR'). Entity and relationship `display_name` values MUST follow the Ontology UI locale above. Attribute names must be safe fields; use English snake_case for English UI and safe Chinese field names for Chinese UI. Description fields and analysis_summary should use the current UI-selected content language specified above."
