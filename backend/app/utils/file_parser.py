...
def _split_plain_text_into_chunks(text: str, chunk_size: int, overlap: int) -> List[str]:
    if chunk_size <= 0:
...
def _is_atomic_markdown_line(line: str) -> bool:
    stripped = line.strip()
    if not stripped:
        return False
...
def _split_markdown_blocks(text: str) -> List[str]:
    ...
    for raw_line in text.splitlines():
        ...
        if not line.strip():
            flush_paragraph()
            continue
        if _is_atomic_markdown_line(line):
            flush_paragraph()
...
def _tail_overlap_blocks(blocks: list[str], overlap: int) -> list[str]:
    ...
    for block in reversed(blocks):
        addition = len(block) + (2 if tail else 0)
...
def split_text_into_chunks(
    ...
    if len(blocks) <= 1:
        return _split_plain_text_into_chunks(text, chunk_size, overlap)

    expanded_blocks: list[str] = []
    for block in blocks:
        if len(block) > chunk_size and not _is_atomic_markdown_line(block):
            expanded_blocks.extend(_split_plain_text_into_chunks(block, chunk_size, overlap))
        else:
    ...
    for block in expanded_blocks:
        ...
        if current_blocks and len(candidate) > chunk_size:
            chunks.append("\n\n".join(current_blocks).strip())
            current_blocks = _tail_overlap_blocks(current_blocks, overlap)
            candidate_blocks = [*current_blocks, block]
            candidate = "\n\n".join(candidate_blocks).strip()
            if current_blocks and len(candidate) > chunk_size:
                current_blocks = []
