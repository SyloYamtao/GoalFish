  ...
  <div class="graph-panel">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 && isGraphEmpty" class="graph-empty-building">
          <div class="loading-spinner"></div>
          <p class="empty-text">{{ $t('graph.buildingGraph') }}</p>
          <p class="empty-hint">{{ $t('graph.waitingGraphData') }}</p>
        </div>
        ...
        <div v-if="selectedItem" class="detail-panel">
          <div class="detail-panel-header">
            ...
            <span v-if="selectedItem.type === 'node'" class="detail-type-badge" :style="{ background: selectedItem.color, color: '#fff' }">
              {{ selectedItem.entityTypeDisplayName }}
            </span>
          ...
          <div v-if="selectedItem.type === 'node'" class="detail-content">
            <div class="detail-row">
              <span class="detail-label">{{ $t('graph.detailName') }}</span>
              <span class="detail-value">{{ selectedItem.data.name }}</span>
            </div>
            <div class="detail-row">
              <span class="detail-label">{{ $t('graph.detailUuid') }}</span>
              <span class="detail-value uuid-text">{{ selectedItem.data.uuid }}</span>
            </div>
            <div class="detail-row" v-if="selectedItem.data.created_at">
              <span class="detail-label">{{ $t('graph.detailCreated') }}</span>
              <span class="detail-value">{{ formatDateTime(selectedItem.data.created_at) }}</span>
            </div>

            <!-- Properties -->
            <div class="detail-section" v-if="selectedItem.data.attributes && Object.keys(selectedItem.data.attributes).length > 0">
              <div class="section-title">{{ $t('graph.detailProperties') }}</div>
            ...
            <div class="detail-section" v-if="selectedItem.data.summary">
              <div class="section-title">{{ $t('graph.detailSummary') }}</div>
              <div class="summary-text">{{ selectedItem.data.summary }}</div>
            </div>

            <!-- Labels -->
            <div class="detail-section" v-if="selectedItem.data.labels && selectedItem.data.labels.length > 0">
              <div class="section-title">{{ $t('graph.detailLabels') }}</div>
              <div class="labels-list">
                <span v-for="label in selectedItem.data.labels" :key="label" class="label-tag">
                  {{ displayEntityType(label) }}
                </span>
              </div>
            </div>
          </div>
          ...
          <div v-else class="detail-content">
            ...
            <template v-if="selectedItem.data.isSelfLoopGroup">
              <div class="edge-relation-header self-loop-header">
                {{ selectedItem.data.source_name }} - {{ $t('graph.selfRelation') }}
                <span class="self-loop-count">{{ $t('graph.itemsUnit', { count: selectedItem.data.selfLoopCount }) }}</span>
              </div>
              ...
              <div class="self-loop-list">
                ...
                >
                  ...
                  >
                    ...
                    <span class="self-loop-name">{{ displayEdgeType(loop.name || loop.fact_type || 'RELATED') }}</span>
                  ...
                  <div class="self-loop-item-content" v-show="expandedSelfLoops.has(loop.uuid || idx)">
                    ...
                    <div class="detail-row" v-if="loop.fact">
                      <span class="detail-label">{{ $t('graph.detailFact') }}</span>
                      <span class="detail-value fact-text">{{ loop.fact }}</span>
                    </div>
                    <div class="detail-row" v-if="loop.fact_type">
                      <span class="detail-label">{{ $t('graph.detailType') }}</span>
                      <span class="detail-value">{{ displayEdgeType(loop.fact_type) }}</span>
                    </div>
                    ...
                    <div v-if="loop.episodes && loop.episodes.length > 0" class="self-loop-episodes">
                      <span class="detail-label">{{ $t('graph.detailEpisodes') }}</span>
            ...
            <template v-else>
              <div class="edge-relation-header">
                {{ selectedItem.data.source_name }} → {{ displayEdgeType(selectedItem.data.name || selectedItem.data.fact_type || 'RELATED_TO') }} → {{ selectedItem.data.target_name }}
              </div>
              ...
              <div class="detail-row">
                <span class="detail-label">{{ $t('graph.detailLabel') }}</span>
                <span class="detail-value">{{ displayEdgeType(selectedItem.data.name || 'RELATED_TO') }}</span>
              </div>
              <div class="detail-row">
                <span class="detail-label">{{ $t('graph.detailType') }}</span>
                <span class="detail-value">{{ selectedItem.data.fact_type ? displayEdgeType(selectedItem.data.fact_type) : $t('common.unknown') }}</span>
              </div>
              ...
              <div class="detail-section" v-if="selectedItem.data.episodes && selectedItem.data.episodes.length > 0">
                <div class="section-title">{{ $t('graph.detailEpisodes') }}</div>
              ...
              <div class="detail-row" v-if="selectedItem.data.valid_at">
                <span class="detail-label">{{ $t('graph.detailValidFrom') }}</span>
      ...
      <!-- 构建中但还没有图谱数据 -->
      <div v-else-if="currentPhase === 1" class="graph-state">
        <div class="loading-spinner"></div>
        <p class="empty-text">{{ $t('graph.buildingGraph') }}</p>
        <p class="empty-hint">{{ $t('graph.waitingGraphData') }}</p>
      </div>
...
import { resolveOntologyEntityLabel, resolveOntologyEdgeLabel } from '../utils/ontologyLabels.js'
import {
  MAX_RENDER_EDGES,
  MAX_RENDER_NODES,
  canRenderEdgeLabels,
...
const isGraphEmpty = computed(() => {
  const nodeCount = props.graphData?.node_count ?? props.graphData?.nodes?.length ?? 0
  const edgeCount = props.graphData?.edge_count ?? props.graphData?.edges?.length ?? 0
  return nodeCount === 0 && edgeCount === 0
})

const entityDisplayNameMap = computed(() => {
  const map = {}
  ;(props.ontology?.entity_types || []).forEach(entity => {
    if (entity?.name) {
...
const edgeDisplayNameMap = computed(() => {
  const map = {}
  ;(props.ontology?.edge_types || props.ontology?.relation_types || []).forEach(edge => {
    if (edge?.name) {
      const displayName = resolveOntologyEdgeLabel(edge, t)
      map[edge.name] = displayName
      map[toPascalCase(edge.name)] = displayName
    }
  })
  return map
})
...
const displayEdgeType = (name) => edgeDisplayNameMap.value[name] || name
...
const entityTypes = computed(() => {
  ...
  props.graphData.nodes.forEach(node => {
    ...
    if (!typeMap[type]) {
      typeMap[type] = {
        name: type,
        displayName: displayEntityType(type),
        count: 0,
...
const isLargeGraph = computed(() => {
  return isLargeGraphData(props.graphData)
})

const getRenderableGraphData = () => {
  const nodesData = props.graphData?.nodes || []
  const edgesData = props.graphData?.edges || []
  if (!isLargeGraph.value) {
    return { nodesData, edgesData, sampled: false }
  }
  ...
  const selectedNodes = [...nodesData]
    .sort((a, b) => (degree.get(b.uuid) || 0) - (degree.get(a.uuid) || 0))
    .slice(0, MAX_RENDER_NODES)
  const selectedIds = new Set(selectedNodes.map(node => node.uuid))
  const selectedEdges = edgesData
...
const renderGraph = () => {
  ...
  const { nodesData, edgesData, sampled } = getRenderableGraphData()
  ...
  const nodes = nodesData.map(n => ({
    id: n.uuid,
    name: n.name || t('graph.unnamedNode'),
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      ...
      edges.push({
        ...
        displayName: t('graph.selfRelationsCount', { count: allSelfLoops.length }),
        curvature: 0,
        isSelfLoop: true,
        rawData: {
    ...
    edges.push({
      ...
      displayName: displayEdgeType(e.name || e.fact_type || 'RELATED'),
      curvature,
      isSelfLoop: false,
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    .force('charge', d3.forceManyBody().strength(sampled ? -180 : -400))
    .force('center', d3.forceCenter(width / 2, height / 2))
    .force('collide', d3.forceCollide(sampled ? 28 : 50))
  ...
  if (sampled) {
    simulation.alphaDecay(0.08).velocityDecay(0.55)
  }
  ...
  const canShowEdgeLabels = canRenderEdgeLabels({ sampled })
  const effectiveShowEdgeLabels = shouldRenderEdgeLabels({
    showEdgeLabels: showEdgeLabels.value,
    sampled,
  })

  const linkLabelBg = canShowEdgeLabels
    ...
    .style('display', effectiveShowEdgeLabels ? 'block' : 'none')
    .on('click', (event, d) => {
      event.stopPropagation()
      linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
      linkLabelBg.attr('fill', 'rgba(255,255,255,0.95)')
    ...
    : linkGroup.selectAll('rect.empty')

  // Link labels
  const linkLabels = canShowEdgeLabels
    ...
    : linkGroup.selectAll('text.empty')
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .on('click', (event, d) => {
      ...
      selectedItem.value = {
        ...
        data: d.rawData,
        entityType: d.type,
        entityTypeDisplayName: displayEntityType(d.type),
...
watch(() => props.ontology, () => {
  scheduleRenderGraph()
})

// 监听边标签显示开关
watch(showEdgeLabels, (newVal) => {
  if (isLargeGraph.value && newVal) {
    showEdgeLabels.value = false
    return
  }
  if (linkLabelsRef) {
