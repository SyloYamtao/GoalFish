    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
        ...
        <div class="card-content">
          <p class="api-note">/api/graph/ontology/generate</p>
          <p class="description">
               ...
               <div class="detail-section" v-if="selectedOntologyItem.source_targets?.length">
                  ...
                  <div class="conn-list">
                     <div v-for="(conn, idx) in selectedOntologyItem.source_targets" :key="idx" class="conn-item">
                        ...
                        <span class="conn-node">{{ displayEntityType(conn.target) }}</span>
                     </div>
                  </div>
               </div>
            </div>
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
        ...
        <div class="card-content">
          <p class="api-note">/api/graph/build</p>
          <p class="description">
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 2, 'completed': currentPhase >= 2 }">
        <div class="card-header">
          ...
          <div class="step-status">
            <span v-if="currentPhase >= 2" class="badge success">{{ $t('common.completed') }}</span>
          </div>
        </div>
...
const regeneratingGraph = ref(false)

const ontologyEdgeTypes = computed(() => (
  props.projectData?.ontology?.edge_types || props.projectData?.ontology?.relation_types || []
))
...
const regenerateStep1 = async () => {
  if (!props.projectData?.project_id || regeneratingGraph.value) return
  regeneratingGraph.value = true
  try {
    await regenerateStepWithConfirm({
