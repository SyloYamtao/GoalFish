  ...
  >
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      <div 
        ...
        :key="project.prediction_run_id || project.project_id"
        class="project-card"
      ...
      >
        <!-- 卡片头部：prediction_run_id 和 功能可用状态 -->
        <div class="card-header">
          <span class="card-id">{{ formatPredictionId(project.prediction_run_id) }}</span>
          <div class="card-status-icons">
            ...
            <span 
              class="status-icon" 
              :class="{ available: project.prediction_run_id, unavailable: !project.prediction_run_id }"
              :title="$t('history.matchScenario')"
            >◎</span>
            <span 
              class="status-icon" 
        ...
        <!-- 卡片标题（使用预测需求的前20字作为标题） -->
        <h3 class="card-title">{{ getPredictionTitle(project.prediction_requirement || project.simulation_requirement) }}</h3>

        <!-- 卡片描述（预测需求完整展示） -->
        <p class="card-desc">{{ truncateText(project.prediction_requirement || project.simulation_requirement, 55) }}</p>
          ...
          <div class="modal-content">
            ...
            <div class="modal-header">
              <div class="modal-title-section">
                <span class="modal-id">{{ formatPredictionId(selectedProject.prediction_run_id) }}</span>
            ...
            <div class="modal-body">
              ...
              <div class="modal-section">
                ...
                <div class="modal-requirement">{{ selectedProject.prediction_requirement || selectedProject.simulation_requirement || $t('common.none') }}</div>
              </div>
            ...
            <div class="modal-actions">
              ...
              <button 
                class="modal-btn btn-scenario" 
                @click="goToScenarioReplay"
                :disabled="!selectedProject.project_id || !selectedProject.prediction_run_id"
              >
...
// 根据预测进度获取样式类
const getProgressClass = (prediction) => {
  const status = prediction.status || ''
  const progress = prediction.progress_percent || 0

  if (status === 'completed') return 'completed'
  if (['failed', 'error'].includes(status)) return 'failed'
  if (status === 'interrupted') return 'in-progress'
...
// 从预测需求生成标题（取前20字）
const getPredictionTitle = (requirement) => {
  if (!requirement) return t('history.untitledSimulation')
  const title = requirement.slice(0, 20)
  return requirement.length > 20 ? title + '...' : title
}

// 格式化 prediction_run_id 显示（截取前6位）
const formatPredictionId = (predictionRunId) => {
  if (!predictionRunId) return 'PRED_UNKNOWN'
  const prefix = predictionRunId.replace('run_', '').slice(0, 6)
  return `PRED_${prefix.toUpperCase()}`
}

// 格式化预测进度
const formatRounds = (prediction) => {
  const status = prediction.status || ''
  const progress = prediction.progress_percent || 0
  if (status === 'completed') return t('history.completed')
  if (status === 'failed' || status === 'error') return t('history.failed')
  if (status === 'interrupted') return t('history.resumable')
  if (progress > 0) return t('history.progressPercent', { percent: progress })
  return t('history.notStarted')
}
...
// 打开项目详情弹窗
const navigateToProject = (prediction) => {
  selectedProject.value = prediction
}
...
// 导航到比赛场景推演页面
const goToScenarioReplay = () => {
  if (selectedProject.value?.project_id && selectedProject.value?.prediction_run_id) {
    router.push({
      name: 'SimulationRun',
      params: {
        projectId: selectedProject.value.project_id,
        predictionRunId: selectedProject.value.prediction_run_id
      }
    })
    closeModal()
  }
...
const loadHistory = async () => {
  try {
    loading.value = true
    const response = await getPredictionHistory(20)
    if (response.success) {
...
.status-icon:nth-child(2).available { color: #F59E0B; } /* 参数建模 - 橙色 */
.status-icon:nth-child(3).available { color: #14B8A6; } /* 场景推演 - 青色 */
.status-icon:nth-child(4).available { color: #10B981; } /* 预测报告 - 绿色 */
.status-icon:nth-child(5).available { color: #6366F1; } /* 预测问答 - 靛蓝 */
...
.modal-btn.btn-scenario .btn-icon { color: #14B8A6; }
.modal-btn.btn-report .btn-icon { color: #10B981; }
.modal-btn.btn-interaction .btn-icon { color: #6366F1; }
